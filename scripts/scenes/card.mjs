...
export function heroCard(total, shipped) {
  ...
  <text x="${L}" y="54" class="mono" font-size="10" fill="${C.ochre}" letter-spacing="2.6">GAME DEVELOPER &#183; PLATFORM ENGINEER &#183; LIVE-OPS</text>
  <text x="${L}" y="100" class="serif" font-size="42" font-weight="700" fill="${C.ink}" letter-spacing="-.6">Alaqmar Gandhi</text></g>`);

  w(`<g opacity="0"><animate attributeName="opacity" from="0" to="1" dur=".6s" begin=".5s" fill="freeze"/>
  <text x="${L}" y="128" class="sans" font-size="14" fill="${C.ink2}">Recruited by a London studio three weeks before my first university lecture.</text>
  <text x="${L}" y="148" class="sans" font-size="14" fill="${C.ink2}">Still going — six shipped titles, and one game that never came out.</text></g>`);

  w(`<line x1="${L}" y1="172" x2="${R}" y2="172" stroke="${C.line}"/>`);
  ...
  for (const s of stats) {
    w(s.count != null
      ? odometer(s.count, { x: s.x, y: 210, cls: 'serif', size: 30, weight: 700, fill: s.color, begin: 1.0, step: 0.08, fmt: M })
      : `<text x="${s.x}" y="210" class="serif" font-size="30" font-weight="700" fill="${s.color}">${s.value}</text>`);
    w(`<text x="${s.x}" y="230" class="mono" font-size="9" fill="${C.ink3}" letter-spacing="1.5">${s.label}</text>`);
  }
  w(`</g>`);
  ...
  <rect x="${n1(bx)}" y="264" width="${n1(X1 - bx)}" height="${LANE_Y[1] + BH + 6 - 264}" rx="4" fill="${C.coral}" fill-opacity=".07"/>
  <line x1="${n1(bx)}" y1="264" x2="${n1(bx)}" y2="${LANE_Y[1] + BH + 6}" stroke="${C.coral}" stroke-opacity=".35" stroke-dasharray="2 3"/>
  <text x="${n1(bx + 6)}" y="259" class="mono" font-size="8.5" fill="${C.coral}" letter-spacing=".8">two roles at once</text></g>`);
  ...
  <line x1="${X1}" y1="264" x2="${X1}" y2="${LANE_Y[1] + BH + 6}" stroke="${C.coral}" stroke-width="1.5"/>
  ...
  <text x="${X1}" y="334" class="mono" font-size="9" fill="${C.coral}" text-anchor="end">now</text></g>`);

  w(`<g class="mono" font-size="9" fill="${C.ink3}">`);
  for (const [label, m] of YEARS) w(`<text x="${n1(px(m))}" y="334">${label}</text>`);
  w(`</g>`);

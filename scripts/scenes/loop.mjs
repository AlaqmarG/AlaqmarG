...
export function loop() {
  const W = 900, H = 196, BW = 146, GAP = 22, X = 48, CY = 116;
  ...
  w(c.open);
  w(`<text x="${X}" y="40" class="mono" font-size="10" fill="${C.ochre}" letter-spacing="2.6">HOW I SHIP</text>
<text x="${X}" y="66" class="serif" font-size="21" font-weight="700" fill="${C.ink}">Nobody publishes by hand</text>`);
  ...
  NODES.forEach((n, i) => {
    ...
    <rect x="${x}" y="${CY - 21}" width="${BW}" height="42" rx="9" fill="${C.paper}"/>
    <rect x="${x}" y="${CY - 21}" width="${BW}" height="42" rx="9" fill="${n.c}" fill-opacity=".10" stroke="${n.c}" stroke-opacity=".45"/>
    <text x="${x + 16}" y="${CY - 2}" class="sans" font-size="12.5" font-weight="600" fill="${C.ink}">${esc(n.t)}</text>
    <text x="${x + 16}" y="${CY + 12}" class="mono" font-size="8.5" fill="${C.ink3}">${esc(n.s)}</text></g>`);
  });

  const arc = `M${last},140 C${last},168 640,176 450,176 C260,176 ${first},168 ${first},140`;
  w(`<path d="${arc}" fill="none" stroke="${C.line}" stroke-width="2" stroke-dasharray="5 5"/>`);
  w(`<path d="M${first - 4},146 l4,-6 l4,6" fill="none" stroke="${C.ink3}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>`);

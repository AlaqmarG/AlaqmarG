...
export function tree() {
  ...
  w(c.open);
  w(`<text x="${L}" y="40" class="mono" font-size="10" fill="${C.ochre}" letter-spacing="2.6">WHAT I WORK WITH</text>
<text x="${L}" y="66" class="serif" font-size="21" font-weight="700" fill="${C.ink}">Five branches</text>
<line x1="${L}" y1="84" x2="852" y2="84" stroke="${C.line}"/>`);

  const ys = BRANCHES.map((_, i) => 120 + i * 56);
  ...
  w(`<circle cx="${ROOT.x}" cy="${ROOT.y}" r="20" fill="${C.paper2}" stroke="${C.ink3}" stroke-opacity=".7"/>

...
export function titles(games, unshipped, stamp) {
  const W = 900, L = 48, R = 852, STEP = 35;
  const rows = [...games].sort((a, b) => b.visits - a.visits);
  const max = rows[0].visits;
  const divY = 104 + rows.length * STEP - 4;
  const H = divY + 36 + (unshipped.length - 1) * 22 + 22;
  ...
  w(c.open);
  w(`<text x="${L}" y="40" class="mono" font-size="10" fill="${C.ochre}" letter-spacing="2.6">SHIPPED</text>
<text x="${L}" y="66" class="serif" font-size="21" font-weight="700" fill="${C.ink}">Six titles, ${(games.reduce((a, g) => a + g.visits, 0) / 1e6).toFixed(1)}M lifetime visits</text>
<text x="${R}" y="66" class="mono" font-size="9.5" fill="${C.ink3}" text-anchor="end">read live from the Roblox API &#183; ${stamp}</text>
<line x1="${L}" y1="84" x2="${R}" y2="84" stroke="${C.line}"/>`);

  rows.forEach((g, i) => {
    const cy = 118 + i * STEP, begin = 0.3 + i * 0.13;
    const barW = (g.visits / max) * (R - L);
    w(`<g opacity="0"><animate attributeName="opacity" from="0" to="1" dur=".4s" begin="${n1(begin)}s" fill="freeze"/>
    <text x="${L}" y="${cy - 1}" class="sans" font-size="13" font-weight="600" fill="${C.ink}">${esc(g.name)}</text>
    <text x="${L}" y="${cy + 13}" class="mono" font-size="9.5" fill="${C.ink3}">${esc(g.studio)}</text>
    ${odometer(g.visits, { x: R, y: cy + 4, cls: 'serif', size: 18, weight: 700, fill: g.color, anchor: 'end', begin: begin + 0.25, step: 0.07 })}
    <rect x="${L}" y="${cy + 17}" width="${R - L}" height="2.5" rx="1.25" fill="${C.line}"/>
    <rect x="${L}" y="${cy + 17}" width="0" height="2.5" rx="1.25" fill="${g.color}" fill-opacity=".85">
  ...
  unshipped.forEach((u, i) => {
    const cy = divY + 24 + i * 22, begin = 1.7 + i * 0.18;
